JJRedConfig = {
    ...
    slowmospeed = 0.25, -- half speed = 0.5
    ...
    flashspeed = 3.15, -- how fast player runs when in slow motion max: 1.15
    AutoShowList = true, -- removing in next release
    powercooldown = 5, -- how long between power uses

}
JJRedConfig.Plugins= {
  LRP = true,-- only for Notifications if perferred
  ESX = true, -- only for Notifications if perferred
  showNotification = true, -- RottonV only for Notifications if perferred
  VORP = false, -- only for Notifications if perferred 
